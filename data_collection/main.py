...
from objs import File, Storage, Author, Work, Institution, Journal, Concept, Affiliation
...
if __name__ == '__main__':
    ...
    if affiliations:
        # set path on local PC
        works_data_dir = (wd.parent / 'openalex-snapshot/data/works').resolve()
        export_dir = Path(wd.parent / 'rae_data/')
        #works_data_dir = Path('D://openalex-snapshot/data/works').resolve()
        ...
        for folder in folders:
            ...
            for item in active_folder_contents:
                if item.is_file():
                    ...
                    compressed_files.append(item_path)
        # initialise File objects into a set
        compressed_files = [Affiliation(f) for f in compressed_files]
        compressed_files = set(compressed_files)
        ...
        if not os.path.exists(export_dir):
            os.mkdir(export_dir)
        # create csv file for local export
        local_export_location = (export_dir / 'affiliations_data.csv')
        # specify variables
        works_vars = ['authod_id', 'paper_id', 'aff_inst_id', 'aff_inst_str', 'year']
        # x_concepts!!!
        local_storage = Storage(local_export_location, works_vars)
        local_storage.init_export_file()
        # create storage for remote export - database, etc.
